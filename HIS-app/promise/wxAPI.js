...
function Request(url, method, data, mark){
    //return Promise Object
    return new Promise(function (resolve, reject) {
        wx.request({
          url: url,
          ...
          header: {
            ...
            "access-token": "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIyIiwiaXNzIjoid3d3LmRlbW8iLCJleHAiOjE2MDk3NjY0MjIsInVzZXJuYW1lIjoibHVoYW4ifQ.Jm9bRKoqApxuHfrKbB3rw9epEksBzJm1uahr0xFno6M"
          },
          method: method,
          success: res => {
              if(res.data.code == 20000){
                console.log("[wxAPI] [网络请求] [%s] success: ", mark, res)
                resolve(res)
              }else {
                console.error("[wxAPI] [网络请求] [%s] fail: ", mark, res)
                reject(res)
              }
          },
          fail: err => {
            console.error("[wxAPI] [网络请求] [%s] fail: ", mark, err)
            reject(err)
          }
        })
    })
